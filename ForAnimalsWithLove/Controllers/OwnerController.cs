...
namespace ForAnimalsWithLove.Controllers
{
    ...
    public class OwnerController : BaseController
    {
		...
		{
			var model = await ownerService.GetHospitalRecordDetailsAsync(id);
			if (model == null)
			{
				return RedirectToAction("Index", "Home");
			} 
		...
		{
			var model = await ownerService.GetGroomingAsync(id);
			if (model == null)
			{
				return RedirectToAction("Index", "Home");
			}
		...
		public async Task<IActionResult> BookingDetails(string id)
		{
			...
			var model = await ownerService.GetBookingDetailsAsync(id);
			if (model == null)
			{
				return RedirectToAction("Index", "Home");
			}
		...
		public async Task<IActionResult> EducationDetails(string id)
		{
			...
			var model = await ownerService.GetEducationDetailsAsync(id);
			if (model == null)
			{
				return RedirectToAction("Index", "Home");
			}
